...
AddEventHandler('onResourceStart', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        ...
        PlayerJob = QBCore.GetPlayerData().job
    end
end)
...
RegisterNetEvent("ls-dispatch:client:AddCallBlip", function(coords, data, blipId)
...
RegisterNetEvent("ls-dispatch:client:removeCallBlip", function(blipId)
...
local respondingCall = nil
RegisterNUICallback("enableRespond", function(data, cb)
	respondingCall = data.callId
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if respondingCall ~= nil then
			
			if IsControlJustPressed(0, 47) then
				TriggerServerEvent("dispatch:respondWithHotkey", respondingCall)
			end
...
RegisterNUICallback("disableDispatchMenu", function(data, cb)
